...
runGWAS <- function(markers, matrix, pheno){
  #create gData object
  ...
  # run GWAS
  GWAS <- runSingleTraitGwas(gData = gData, thrType = "bonf")
  
  # signSNP
  signSNP <- as.data.frame(GWAS$signSnp)
  signSNPTable <- signSNP
  if (nrow(signSNP) > 0 && ncol(signSNP) > 0){
...
plotsGWAS <- function(gwas, var, ythr = NULL, chr = NULL){
  qqPlot <- plot(gwas, plotType = "qq", trait = var, title = var)
  manPlot <- plot(gwas, plotType = "manhattan", trait = var, yThr = ythr, chr = chr, title = var)
  return(list(qqPlot = qqPlot, manPlot = manPlot))
}

plotsGWAS2 <- function(gwas, var, ythr = NULL, chr = NULL){
  qqPlot <- plot(gwas, plotType = "qq", trait = var, title = var)
  manPlot <- plot(gwas, plotType = "manhattan", trait = var, yThr = ythr, chr = chr, title = var)
}
